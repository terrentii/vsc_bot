...
from bot.api_client import VSCAPIClient
from bot.utils import parse_msg_command, parse_login_command, format_room_list, format_message
...
async def cmd_login(message: Message, state: FSMContext):
    ...
    # TODO: Здесь должна быть реальная проверка логина/пароля на сервере
    # и получение API-ключа. Пока просто сохраняем логин.
...
async def cmd_msg(message: Message):
    ...
    full_text = f"{text}\n\n— отправлено через {display_name}"
    
    success, result = await api.send_message(room_id, full_text)
...
async def process_media(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        if success:
            display_name = get_display_name(tg_id)
            text = f"— отправлено через {display_name}"
            
            msg_success, msg_result = await api.send_message(
                room_id, text, media=server_filename
            )
            ...
            if msg_success:
                await status_msg.edit_text(
                    ...
                    f"Файл: <code>{server_filename}</code>",
                    parse_mode="HTML"
                )
            else:
                await status_msg.edit_text(
                    f"⚠️ Файл загружен, но не удалось отправить сообщение.\n"
                    f"Файл: <code>{server_filename}</code>",
                    parse_mode="HTML"
                )
        else:
            await status_msg.edit_text(
                ...
                "Возможно, превышен размер или недопустимый формат.",
                parse_mode="HTML"
            )
...
async def cmd_searchroom(message: Message):
    ...
    if len(parts) < 2:
        await message.answer(
            ...
            'Используйте: <code>/searchroom \"Название комнаты\"</code>',
            parse_mode="HTML"
        )
        return
    ...
    if found:
        text = format_room_list(found, 10)
    else:
...
async def cmd_stalkroom(message: Message, state: FSMContext, bot: Bot):
    ...
    await message.answer(
        ...
        f"Новые сообщения будут приходить сюда автоматически через WebSocket.\n"
        f"Для остановки используйте <code>/unstalk 1234567890</code>",
        parse_mode="HTML"
    )
...
async def cmd_unstalk(message: Message):
    parts = message.text.split(maxsplit=1)
    if len(parts) < 2:
        await message.answer("❌ Укажите ID комнаты: <code>/unstalk 1234567890</code>", parse_mode="HTML")
        return
    ...
    if room_id in info["stalking"]:
        ...
        await message.answer(f"✅ Слежение за комнатой <code>{room_id}</code> остановлено.", parse_mode="HTML")
    else:
